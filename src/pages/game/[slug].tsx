...
export default function Index(props: GameTemplateProps) {
  ...
  // você pode mostrar um loading ou uma tela de esqueleto
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const TODAY = new Date().toISOString().slice(0, 10) // 2021-10-24
  const { data: dataUpcoming } = await apolloClient.query<
    QueryUpcoming,
    QueryUpcomingVariables
  >({
  ...
  return {
    ...
    props: {
      ...
      upcomingTitle: dataUpcoming.showcase?.upcomingGames?.title,
      upcomingGames: gamesMapper(dataUpcoming.upcomingGames),
      upcomingHighlight: highlightMapper(
        dataUpcoming.showcase?.upcomingGames?.highlight
      ),
